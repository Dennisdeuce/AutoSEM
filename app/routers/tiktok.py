...
Ad creation strategy priority (v0.4.0 - TT_USER Push Spark Ads):
1. SINGLE_VIDEO + TT_USER (Push Spark Ad - TikTok's native format)
2. SINGLE_IMAGE + TT_USER (Push Spark Ad - image fallback)
3. SINGLE_IMAGE + display_name only (legacy non-identity fallback)
4. Pangle display ad (audience network)

Identity types (as of early 2026):
- TT_USER: Linked TikTok account. Required for all ads now that Custom Identity is deprecated.
  Used for "Push Spark Ads" - upload new creative that gets published under linked account.
- BC_AUTH_TT: Business Center authorized accounts (alternative to TT_USER)
- CUSTOMIZED_USER: DEPRECATED by TikTok in early 2026. No longer works for ad creation.

Push Spark Ads: Upload new video/image to Ads Manager, create ad with TT_USER identity.
  The creative gets published under the linked TikTok Business Account (private mode).
  No tiktok_item_id needed - that's only for "Pull" Spark Ads (existing organic posts).
"""
...
def _find_best_identity(access_token: str, advertiser_id: str) -> dict:
    ...
    As of early 2026, TikTok has deprecated CUSTOMIZED_USER (Custom Identity).
    All ads now require a linked TikTok account.

    Priority: TT_USER > BC_AUTH_TT > CUSTOMIZED_USER (deprecated, last resort)
    """
    # Priority 1: TT_USER (linked TikTok account - required for Push Spark Ads)
    ...
    if result.get("code") == 0:
        ...
        if identities:
            ident = identities[0]
            logger.info(f"Using TT_USER identity: {ident.get('identity_id')} ({ident.get('display_name')}) - Push Spark Ads")
    ...
    # Priority 3: CUSTOMIZED_USER (deprecated - last resort, will likely fail)
    result = _tiktok_api("GET", "/identity/get/", access_token,
    ...
    if result.get("code") == 0:
        ...
        if identities:
            ident = identities[0]
            logger.warning("Using CUSTOMIZED_USER identity as last resort - DEPRECATED by TikTok")
            return {"identity_id": ident.get("identity_id"),
...
def _try_create_ad(access_token: str, advertiser_id: str, adgroup_id: str,
    ...
    As of early 2026, TikTok requires TT_USER identity (linked TikTok account).
    CUSTOMIZED_USER is deprecated and returns "Custom identities are no longer supported."

    Push Spark Ads: Upload new creative + create ad with TT_USER = Spark Ad published
    under linked TikTok Business Account. No tiktok_item_id needed.

    Strategy 1: SINGLE_VIDEO + TT_USER (Push Spark Ad - TikTok native video)
    Strategy 2: SINGLE_IMAGE + TT_USER (Push Spark Ad - image)
    Strategy 3: SINGLE_IMAGE + display_name only (non-identity legacy fallback)
    Strategy 4: Pangle display network (image ad)
    """
    ...
    # ── Strategy 1: SINGLE_VIDEO + TT_USER (Push Spark Ad - TikTok native) ──
    if video_id and identity_id and identity_type in ("TT_USER", "BC_AUTH_TT"):
        creative = {
        ...
        attempts.append({"strategy": "video_tt_user_spark", "code": result.get("code"),
        ...
        if result.get("code") == 0:
            return {"success": True, "ad_ids": result.get("data", {}).get("ad_ids", []),
                    "strategy": "video_tt_user_spark", "attempts": attempts}

    # ── Strategy 2: SINGLE_IMAGE + TT_USER (Push Spark Ad - image) ──
    if image_id and identity_id and identity_type in ("TT_USER", "BC_AUTH_TT"):
        creative = {
        ...
        result = _tiktok_api("POST", "/ad/create/", access_token, data={
            "advertiser_id": advertiser_id, "adgroup_id": adgroup_id,
            "creatives": [creative], "operation_status": "ENABLE"})
        attempts.append({"strategy": "image_tt_user_spark", "code": result.get("code"),
                        "message": result.get("message"), "data": result.get("data"),
                        "identity_type_used": identity_type})
        if result.get("code") == 0:
            return {"success": True, "ad_ids": result.get("data", {}).get("ad_ids", []),
                    "strategy": "image_tt_user_spark", "attempts": attempts}

    # ── Strategy 3: SINGLE_IMAGE + display_name only (non-identity fallback) ──
    if image_id:
        creative = {
        ...
        result = _tiktok_api("POST", "/ad/create/", access_token, data={
            "advertiser_id": advertiser_id, "adgroup_id": adgroup_id,
            "creatives": [creative], "operation_status": "ENABLE"})
        attempts.append({"strategy": "image_display_name_only", "code": result.get("code"),
                        "message": result.get("message"), "data": result.get("data")})
        if result.get("code") == 0:
            return {"success": True, "ad_ids": result.get("data", {}).get("ad_ids", []),
                    "strategy": "image_display_name_only", "attempts": attempts}

    # ── Strategy 4: Pangle display network (image ad with location_ids) ──
...
def launch_campaign(daily_budget: float = Query(20.0),
    ...
    try:
        ...
        # Step 5: Find identity (TT_USER preferred - Push Spark Ads)
...
def create_ad_for_adgroup(adgroup_id: str = Query(...),
                          ...
                          db: Session = Depends(get_db)):
    """Create an ad for an existing ad group using Push Spark Ads (TT_USER)."""
    creds = _get_active_token(db)
    if not creds["access_token"] or not creds["advertiser_id"]:
        return {"success": False, "error": "TikTok not connected"}
    access_token, advertiser_id = creds["access_token"], creds["advertiser_id"]
    ...
    # Generate video (may fail if no ffmpeg - that's OK, image strategies will be used)
    ...
    # Identity (TT_USER preferred for Push Spark Ads)
    identity = _find_best_identity(access_token, advertiser_id)
    steps.append({"step": "identity", "result": identity})
    if not identity.get("identity_id"):
        return {"success": False, "error": "No TT_USER identity found. Link a TikTok account in TikTok Ads Manager > Business Center.", "steps": steps}
