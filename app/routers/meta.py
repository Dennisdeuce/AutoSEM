...
def oauth_callback(code: str = Query(None), error: str = Query(None), db: Session = Depends(get_db)):
    if error:
    ...
    try:
        ...
        long_url = (
            ...
            f"&client_secret={META_APP_SECRET}&fb_exchange_token={short_token}"
        )
        ...
        return HTMLResponse(content="<h1>Meta Connected!</h1><p>Long-lived token saved.</p>")
...
def check_meta_status(db: Session = Depends(get_db)):
    ...
    try:
        resp = requests.get(f"https://graph.facebook.com/v19.0/me?access_token={token.access_token}", timeout=10)
        if resp.status_code == 200:
            return {"connected": True, "token_type": token.token_type,
...
def refresh_meta_token(db: Session = Depends(get_db)):
    ...
    try:
        url = (
            f"https://graph.facebook.com/v19.0/oauth/access_token"
            f"?grant_type=fb_exchange_token&client_id={META_APP_ID}"
            f"&client_secret={META_APP_SECRET}&fb_exchange_token={token.access_token}"
        )
